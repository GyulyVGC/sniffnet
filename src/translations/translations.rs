...
pub fn choose_adapters_translation(language: Language) -> Text<'static, Renderer<StyleType>> {
    Text::new(match language {
        ...
        Language::JP => "使用するネットワーク アダプターを選択してください",
    })
}
...
pub fn select_filters_translation(language: Language) -> Text<'static, Renderer<StyleType>> {
    Text::new(match language {
        ...
        Language::JP => "トラフィックに適用するフィルターを選択してください",
    })
}
...
pub fn ask_clear_all_translation(language: Language) -> Text<'static, Renderer<StyleType>> {
    Text::new(match language {
        ...
        Language::JP => "すべての通知を削除します。よろしいですか？",
    })
}
...
pub fn no_addresses_translation(
    ...
    Text::new(match language {
        ...
        Language::JP => format!("選択されたアダプターが有効なアドレスを持っていないため、トラフィックを観測できていません...\n\n\
...
pub fn waiting_translation(
    ...
    Text::new(match language {
        ...
        Language::JP => format!("トラフィックがまだ観測できていません。ネットワーク パケットを待っています...\n\n\
                                 ネットワーク アダプター: {adapter}\n\n\
...
pub fn some_observed_translation(
    ...
    Text::new(match language {
        ...
        Language::JP => format!("取得したパケット数: {observed}\n\n\
                                 フィルター後のパケット数: 0\n\n\
...
pub fn of_total_translation(language: Language, percentage: &str) -> String {
    match language {
        ...
        Language::JP => format!("(トータル: {percentage} )"),
    }
}
...
pub fn yeti_night_translation(language: Language) -> &'static str {
    match language {
        ...
        Language::JP => "Sniffnet のオリジナル テーマ",
    }
}
...
pub fn yeti_day_translation(language: Language) -> &'static str {
    match language {
        ...
        Language::JP => "Sniffnet のオリジナル ライト テーマ",
    }
}
...
pub fn deep_sea_translation(language: Language) -> &'static str {
    match language {
        ...
        Language::JP => "ネットワーク トラフィックにダイブする",
    }
}
...
pub fn packets_threshold_translation(language: Language) -> &'static str {
    match language {
        ...
        Language::JP => "パケット数の閾値を超過した場合に通知する",
    }
}
...
pub fn bytes_threshold_translation(language: Language) -> &'static str {
    match language {
        ...
        Language::JP => "バイト量の閾値を調査した場合に通知する",
    }
}
...
pub fn favorite_notification_translation(language: Language) -> &'static str {
    match language {
        ...
        Language::JP => "お気に入りに指定したホストに関してデータ送受信があった場合に通知する",
    }
}
...
pub fn bytes_exceeded_value_translation(language: Language, value: &str) -> String {
    match language {
        ...
        Language::JP => format!("{value} の送受信が発生しました"),
    }
}
...
pub fn packets_exceeded_value_translation(language: Language, value: u32) -> String {
    match language {
        ...
        Language::JP => format!("{value} パケットの送受信が発生しました"),
    }
}
...
pub fn favorite_transmitted_translation(language: Language) -> &'static str {
    match language {
        ...
        Language::JP => "お気に入りのホストで新しいデータ送受信が発生しました!",
    }
}
...
pub fn only_last_30_translation(language: Language) -> &'static str {
    match language {
        ...
        Language::JP => "最新の通知 30 件のみ表示されます",
    }
}
